...
def run_automation():
    ...
    try:
        subprocess.run([sys.executable, "app.py", "--mode", "auto"], check=True)
    except subprocess.CalledProcessError as e:
        print(f"{Fore.RED}❌ 자동화 모드 실행 실패: {e}{Style.RESET_ALL}")

def run_single_analysis():
    """즉시 분석 모드 실행"""
    print(f"{Fore.GREEN}⚡ 즉시 분석을 시작합니다...{Style.RESET_ALL}")
    try:
        subprocess.run([sys.executable, "app.py", "--mode", "single"], check=True)
    except subprocess.CalledProcessError as e:
        print(f"{Fore.RED}❌ 즉시 분석 실행 실패: {e}{Style.RESET_ALL}")

def check_system():
    """시스템 상태 체크"""
    print(f"{Fore.GREEN}📊 시스템 상태를 확인합니다...{Style.RESET_ALL}")
    try:
        subprocess.run([sys.executable, "check_system_specs.py"], check=True)
    except subprocess.CalledProcessError as e:
        print(f"{Fore.RED}❌ 시스템 체크 실행 실패: {e}{Style.RESET_ALL}")

def run_optimization():
    """성능 최적화 모드"""
    print(f"{Fore.GREEN}🔧 성능 최적화를 시작합니다...{Style.RESET_ALL}")
    try:
        subprocess.run([sys.executable, "run_analysis.py"], check=True)
    except subprocess.CalledProcessError as e:
        print(f"{Fore.RED}❌ 최적화 모드 실행 실패: {e}{Style.RESET_ALL}")

def run_test_mode():
    """테스트 모드 (간단 분석)"""
    print(f"{Fore.GREEN}📋 테스트 모드를 시작합니다...{Style.RESET_ALL}")
    try:
        # 백업 폴더에서 simple_run을 실행
        subprocess.run([sys.executable, "backup_old_files/simple_run.py"], check=True)
    except subprocess.CalledProcessError as e:
        print(f"{Fore.RED}❌ 테스트 모드 실행 실패: {e}{Style.RESET_ALL}")
...
def main():
    ...
    while True:
        ...
        try:
            choice = input(f"{Fore.CYAN}선택 (1-6): {Style.RESET_ALL}").strip()
