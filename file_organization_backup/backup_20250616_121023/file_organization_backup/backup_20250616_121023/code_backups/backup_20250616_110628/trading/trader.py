...
class Trader:
    ...
    async def run_trading_cycle(self):
        ...
        try:
            ...
            # 실제 거래 로직은 ScalpingTrader를 사용
            # 여기서는 간단한 상태 체크만 수행
            
            await asyncio.sleep(1)  # 비동기 대기
            
        except Exception as e:
            log_event("ERROR", f"거래 사이클 오류: {e}")
    ...
    async def _start_new_audition(self):
        ...
        try:
            candidates = self.fetcher.find_market_prism_candidates(top_n=config.AUDITION_CANDIDATE_COUNT)
            if not candidates:
                log_event("WARNING", "[신규 오디션] 마켓 프리즘 후보 종목이 없습니다.")
                return

            # 척후병 1주씩 파견 (현금 25% 유지 조건 자동 적용)
            prices = await self.kis_api.fetch_prices_in_parallel(candidates)
            buy_tasks = []
    ...
    async def _execute_buy(self, ticker: str, price: float, quantity: int, promote: bool = False):
        ...
        # Throttle 적용된 주문
        order_result = await self.kis_api.throttled_order('buy', ticker, quantity)
        if order_result and order_result.get('order_id'):
            ...
            self.telegram_bot.send_message(f"🔥 [매수 오류] {ticker} | 사유: {order_result}")

    async def _execute_sell(self, stock: Stock, price: float, reason: str):
        """(비동기) 매도 주문 및 포트폴리오 업데이트"""
        order_result = await self.kis_api.throttled_order('sell', stock.ticker, stock.quantity)
        if order_result and order_result.get('order_id'):
            ...
            self.telegram_bot.send_message(f"🔥 [매도 오류] {stock.ticker} | 사유: {order_result}") 