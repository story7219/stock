...
def quick_connection_test():
    ...
    try:
        ...
        # 4. Gemini 1.5 Flash API 테스트 (고정)
        import google.generativeai as genai
        
        genai.configure(api_key=gemini_key)
        model = genai.GenerativeModel('gemini-1.5-flash')  # 고정
        
        response = model.generate_content("안녕하세요, Gemini 1.5 Flash 테스트입니다")
        if response and response.text:
            print("✅ Gemini 1.5 Flash API 연결 성공 (가성비 최적화)")
        else:
